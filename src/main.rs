...
use iced::{
    ...
    Element, Length, Application, Settings, Theme, executor, Command
};
use kira::{
...
use crate::{
    audio::{
        play_track, toggle_play, change_volume, get_music_list,
    },
...
impl Application for AmaiPlayer {
    ...
    fn view(&self) -> iced::Element<'_, Self::Message> {
        let play_button =
            button(text(if self.is_playing { "⏸" } else { "▶" }).shaping(text::Shaping::Advanced)).on_press(Message::TogglePlay);
        ...
        let music_buttons: Element<_> = self
            ...
            .fold(
                ...
                |column, (index, file)| {
                    ...
                    let button_text = text(file_name).shaping(
                        match contains_non_english_chars(file_name) {
        ...
        let music_list_container = container(music_list).width(Length::Fill).height(Length::Fill);
        ...
        container(final_content).width(Length::Fill).height(Length::Fill).into()
    }
}
